...
## What it shows
A compact 9-bar meter styled like the top bar (same panel shape and opacity):
...
- Bars ramp **yellow → orange → red** as the imbalance grows. The converter icon marks the idle-converters side, the energy bolt marks the overflow side.
- The E/s value of the imbalance is always shown in a fixed-width slot (no jumping as digits change). At 3+ bars a plain yellow hint appears: **Overflowing** or **Idle converters**.
